...
class Button(QtWidgets.QPushButton):
    def __init__(self, name, parent=None):
        super(Button, self).__init__(name, parent=parent)
        self.setMouseTracking(True)
        self.setStyleSheet("""
        QPushButton
        {
...
if __name__ == "__main__":
    ...
    #     if self._summonPosition != self._currentMousePos:
    #         lineCenterToMouse = QtCore.QLineF(self._summonPosition, self._currentMousePos)
    #         normLine = lineCenterToMouse.unitVector() # Create a line with the same origine and direction but with a length of 1
    #         finalPointF = normLine.p1() + (normLine.p2() - normLine.p1())*self._inRadius
    #         finalPoint = QtCore.QPoint(int(finalPointF.x()), int(finalPointF.y()))
    #         angle = QtCore.QLineF(self._summonPosition, self._summonPosition+QtCore.QPoint(self._inRadius, 0)).angleTo(normLine)
    ...
    #         line = QtCore.QLineF(self._summonPosition, self._summonPosition+QtCore.QPoint(self._outRadius, 0))
    #         targetAngle = counter*(360/len(self._btnList))
    #         line.setAngle(targetAngle)
    #         lineToBtn[line.toTuple()] = btn
    #         if targetLine is None:
    #             targetLine = line.toTuple()

    #         elif refLine.angleTo(line) < refLine.angleTo(QtCore.QLineF(*targetLine)):
    #             targetLine = line.toTuple()
    ...
    #     if mouseInCircle or not self._animFinished:
    #         targetLine = None
            
    #     if targetLine and self._animFinished is True and not mouseInCircle:
    #         painter.setPen(QtGui.QPen(QtCore.Qt.yellow, 5))
    #         painter.drawLine(*targetLine)
    #         self._selectedBtn = lineToBtn[targetLine]
    #         self._selectedBtn.setHover(True)
    #         if self._mousePressed is True:
    #             self._selectedBtn.setPress(True)
    #     else:            
